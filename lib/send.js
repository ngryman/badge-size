...
function log(res, baton) {
  ...
  logger.info({
    badge: {
      ...
      style: baton.style,
      max: baton.max,
      softmax: baton.softmax,
      value: baton.value,
      extension: baton.extension,
      originalSize: baton.originalSize,
      size: baton.size,
      compression: baton.compression
    },
    req: {
      clientIp,
    ...
    res: {
      ...
      time: getDuration(baton.time)
    }
  }, `${req.method} ${res.statusCode} ${req.url} ${clientIp} ${userAgent}`)
}
